...
string BuildSnapshotPayload(string symbol, int bars)
{
   ...
   {
      candles += StringFormat("{\"time\":\"%s\",\"open\":%.2f,\"high\":%.2f,\"low\":%.2f,\"close\":%.2f,\"tick_volume\":%d}",
...
void ExecuteCommandIfAny(string raw)
{
   ...
   double volume = StrToDouble(JsonExtract(raw, "volume"));
   double sl = StrToDouble(JsonExtract(raw, "sl"));
   double tp = StrToDouble(JsonExtract(raw, "tp"));
   double price = StrToDouble(JsonExtract(raw, "price"));
...
string JsonExtract(string src, string key)
{
   ...
   while(start < StringLen(src) && (StringGetCharacter(src, start) == ' ' || StringGetCharacter(src, start) == '"')) start++;

   int end = start;
   while(end < StringLen(src))
   {
      ushort c = (ushort)StringGetCharacter(src, end);
      if(c == ',' || c == '}' || c == '"') break;
      end++;
   }
